...
2026-07-21 팀원 전달 자료 대조 결과, `etf_bydd_trd (1).zip`은 기존 전달본과
SHA-256이 `402DF6BBFE7161178A715E80AE157AF01D7D51655281BEA936C9DD75409FE1EC`로
동일했고 내부 KRX 일별 원본 1,704개도 작업공간 파일과 모두 같았다. 따라서 기존
원본을 덮어쓰지 않았다. `classification.zip`의 9개 파일은 같은 날 전달된
`data (1).zip` 내부 파일과 CRC·크기가 모두 같아 중복 적재하지 않았고, 통합
자료에서는 작업공간에 없던 캐시·원본만 복원했다. 기존 파일과 내용이 다른
항목은 현재 main의 더 최신 코드·목데이터·승인 문서를 유지했다.
