...
챗봇은 이 정규화 캐시를 구성종목 질문의 동적 근거로 사용한다. 테마 문서에 비중을 복사하지 않으며, 응답 시 스냅샷의 `snapshot_date`, ETF 종목코드와 구성종목 행의 `stck_shrn_iscd`, `hts_kor_isnm`, `etf_cnfg_issu_rlim`을 함께 연결한다. 상세 가드레일은 [ETF 테마 챗봇 계약](./ETF_테마_챗봇_계약.md)을 따른다.
