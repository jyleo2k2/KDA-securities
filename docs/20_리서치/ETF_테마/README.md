...
| 번호 | 테마 | 런타임 ID | 기본 역할 |
|---:|---|---|---|
| 1 | 반도체 | `semiconductor` | 전술 테마 |
| 2 | 신재생·친환경 | `renewable_green` | 전술 테마 |
| 3 | 바이오·헬스케어 | `bio_healthcare` | 전술 테마 |
| 4 | 2차전지·배터리 | `secondary_battery` | 전술 테마 |
| 5 | 건설·기계·인프라 | `construction_machinery_infra` | 전술 테마 |
| 6 | 자동차·모빌리티 | `automotive_mobility` | 전술 테마 |
| 7 | 그룹주 | `business_groups` | 전술 테마 |
| 8 | 에너지·정유 | `energy_refining` | 전술 테마 |
| 9 | 미디어·엔터·게임 | `media_entertainment_gaming` | 전술 테마 |
| 10 | 원자력·전력 | `nuclear_power_grid` | 전술 테마 |
| 11 | 리츠·부동산 | `reit_real_estate` | 실물자산 |
| 12 | 로봇 | `robotics` | 전술 테마 |
| 13 | 은행·금융 | `bank_finance` | 전술 테마 |
| 14 | 방산·우주 | `defense_space` | 전술 테마 |
| 15 | 소비재·음식료 | `consumer_food` | 전술 테마 |
| 16 | 금·원자재 | `gold_commodities` | 실물자산 |
| 17 | 철강·소재 | `steel_materials` | 전술 테마 |
| 18 | 양자컴퓨팅 | `quantum_computing` | 전술 테마 |
| 19 | 메타버스 | `metaverse` | 전술 테마 |
| 20 | 조선 | `shipbuilding` | 전술 테마 |
| 21 | 채권 | `bonds` | 자본보전 |

각 테마의 쉬운 설명, 포함 분야, 관찰 요인, 한 줄 비유, 정의, 동의어, 분류 포함·제외어, 장점과 위험은 JSON 카탈로그에서 관리한다. 카탈로그 `2026-07-22.1`은 21개 테마마다 성과 관찰요인 3개를 독립 항목으로 관리하고, 각 항목에 해당 요인이 산업의 주문·매출·비용·가동률 등에 연결되는 이유를 함께 기록한다. ETF 하나가 여러 테마 조건에 맞을 수 있으므로 분류는 다대다 관계다.

챗봇은 테마 질문을 `개요`, `대표기업`, `투자 고려사항`, `성과 관찰요인`, `고유 위험`으로 구분한다. 대표기업은 테마를 이해시키는 교육용 예시이고, 실제 ETF의 구성종목·비중은 운용사 공식 공시 스냅샷에서 별도로 조회한다.
...
- 승인 대상 문구의 단일 원본은 카탈로그 `2026-07-22.1`이다. 이 문서는 승인 범위·출처 계보·운영 경계를 기록하며 승인 RAG 매니페스트에 등록한다.
...
- 구성종목과 비중은 이 문서에 복사하지 않는다. 운용사 공식 공시의 기준일별 구조화 스냅샷을 조회해 별도 출처 칩과 함께 표시한다.
...
4. 원격 반영: 승인 지식 적재·임베딩 후 105개 질문 유형의 payload 해시와 실제 지식 청크·공식 URL을 검증 장부에 연결한다.
...
위 링크는 ETF의 공통 구조와 비교지표를 뒷받침한다. 21개 테마별 산업 설명의 출처 계보는 아래 조사자료에 포함된 공식 링크와 카탈로그의 대표기업 공식 홈페이지 URL로 보존한다. 공유 대화 URL 자체는 공식 발행기관 출처 칩으로 사용하지 않는다.
...
### [theme:bonds] 채권
[한국거래소 ETF 투자자 안내](https://open.krx.co.kr/contents/OPN/01/01030100/OPN01030100.jsp)
