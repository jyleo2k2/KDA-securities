...
> 승인 범위: 카탈로그 `2026-07-20.4`의 23개 테마와 질문 유형 5종
> 검토 승인: 이호연(조장), 이재용(총괄)
...
각 테마의 쉬운 설명, 포함 분야, 관찰 요인, 한 줄 비유, 정의, 동의어, 분류 포함·제외어, 장점과 위험은 JSON 카탈로그에서 관리한다. 카탈로그 `2026-07-20.4`부터는 23개 테마마다 성과 관찰요인 3개를 독립 항목으로 관리하고, 각 항목에 해당 요인이 산업의 주문·매출·비용·가동률 등에 연결되는 이유를 함께 기록한다. 카탈로그 스키마 v3부터는 테마별 대표기업 3곳의 역할·초보자용 설명·선정 이유·기업 공식 홈페이지·기준일도 함께 관리한다. ETF 하나가 여러 테마 조건에 맞을 수 있으므로 분류는 다대다 관계다.
...
- 승인 대상 문구의 단일 원본은 카탈로그 `2026-07-20.4`이다. 이 문서는 승인 범위·출처 계보·운영 경계를 기록하며 승인 RAG 매니페스트에 등록한다.
