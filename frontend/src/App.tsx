...
export default function App(): JSX.Element {
  ...
  const [surveyProfile, setSurveyProfile] = useState<CompletedSurveyProfile | null>(() => {
    ...
    const storedVersion = window.localStorage.getItem("pension-copilot:mvp-profile-version");
    if (stored && storedVersion === SURVEY_PROFILE_VERSION) {
      try { return JSON.parse(stored) as CompletedSurveyProfile; } catch { /* cleared below */ }
    }
    clearUserStorage();
    return null;
  });
  ...
  const showLogin = activeRoute === "login" || auth.session === null;
  ...
  useEffect(() => {
    ...
    const authChanged = userChanged || previousAuth?.token !== accessToken;
    if (!authChanged) return;
    previousAuthRef.current = { userId: authenticatedUserId, token: accessToken };
    const loadGeneration = ++userLoadGenerationRef.current;
    if (userChanged) {
      clearUserStorage();
    ...
    if (!accessToken) {
      setCurrentUserData({ context: null, hero: null, loading: false, error: null });
    ...
    void Promise.all([getMyPensionContext(accessToken), getDemoHeroes()])
      .then(([context, heroes]) => {
        if (userLoadGenerationRef.current !== loadGeneration) return;
      ...
      .catch((error: unknown) => {
        if (userLoadGenerationRef.current !== loadGeneration) return;
        setCurrentUserData({ context: null, hero: null, loading: false, error: pensionContextErrorMessage(error) });
      });
  }, [accessToken, activeRoute, auth.loading, authenticatedUserId]);
  ...
  function completeSurvey(profile: CompletedSurveyProfile): void {
    window.localStorage.setItem("pension-copilot:survey-profile", JSON.stringify(profile));
    window.localStorage.setItem("pension-copilot:mvp-profile-version", SURVEY_PROFILE_VERSION);
  ...
  if (auth.loading) return <main style={{ display: "grid", minHeight: "100vh", placeItems: "center" }}>로그인 상태를 확인하고 있습니다…</main>;

  const displayName = currentUserData.context?.nickname ?? auth.session?.user.email?.replace("@kda-demo.invalid", "") ?? "인증 사용자";
  return (
    <>
      {showLogin ? (
        <LoginFlowPage auth={auth} onStart={() => changeTab("home")} />
      ) : activeTab === "guide" ? (
        <div className="guide-tab"><GuidePage auth={auth} initialScenarioCode={selectedScenarioCode} onSignOut={handleSignOut} surveyProfile={surveyProfile} userContext={currentUserData.context} /></div>
      ) : (
        <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", paddingBottom: 72, fontFamily: "system-ui, sans-serif" }}>
          ...
          <main style={{ padding: 16 }}>
            {activeTab === "home" ? <HomePage error={currentUserData.error} hero={currentUserData.hero} loading={currentUserData.loading} onAnalyzeHero={analyzeHero} userContext={currentUserData.context} /> : activeTab === "profile" ? <ProfilePage profile={surveyProfile} onComplete={completeSurvey} userContext={currentUserData.context} /> : CardPage ? <CardPage /> : null}
          </main>
        </div>
      )}
      {!showLogin && <TabBar activeTab={activeTab} onChange={changeTab} />}
    </>
  );
}
