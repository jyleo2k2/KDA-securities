...
interface CurrentUserData { context: DemoUserFinancialContext | null; hero: DemoHeroPortfolio | null; loading: boolean; error: string | null; }
function routeFromHash(): AppRoute { const candidate = window.location.hash.slice(1) as AppRoute; return candidate === "login" || TAB_KEYS.includes(candidate as TabKey) ? candidate : "login"; }
function clearUserStorage(): void { USER_STORAGE_KEYS.forEach((key) => window.localStorage.removeItem(key)); }
function pensionContextErrorMessage(error: unknown): string { if (error instanceof ApiError && error.status === 404) return "이 계정에는 연동된 연금 데이터가 없습니다."; if (error instanceof ApiError && error.status === 503) return "연금 데이터를 불러오는 서비스에 연결할 수 없습니다. 잠시 후 다시 시도해 주세요."; return "연금 데이터를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요."; }
...
export default function App(): JSX.Element {
  ...
  const [surveyProfile, setSurveyProfile] = useState<CompletedSurveyProfile | null>(() => {
    ...
    if (stored && version === SURVEY_PROFILE_VERSION) { try { return JSON.parse(stored) as CompletedSurveyProfile; } catch { /* clear below */ } }
    clearUserStorage(); return null;
  });
  ...
  useEffect(() => { const sync = () => setActiveRoute(routeFromHash()); window.addEventListener("hashchange", sync); return () => window.removeEventListener("hashchange", sync); }, []);
  ...
  useEffect(() => {
    ...
    void Promise.all([getMyPensionContext(accessToken), getDemoHeroes()]).then(([context, heroes]) => {
    ...
    }).catch((error: unknown) => { if (userLoadGenerationRef.current === generation) setCurrentUserData({ context: null, hero: null, loading: false, error: pensionContextErrorMessage(error) }); });
  ...
  function completeLogin(): void { setLoginSuccessPending(false); changeTab("home"); }
  ...
  if (auth.configured && (!auth.session || loginSuccessPending)) return <LoginFlowPage auth={auth} onAuthenticated={() => setLoginSuccessPending(true)} onStart={completeLogin} />;
