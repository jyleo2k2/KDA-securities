...
export function GuidePage({
  ...
  const {
    ...
    pensionTaxInput,
    surveyProfile,
    blocked: deletingSessionId !== null,
    onResponse: (response, result) => {
      setConversationContext(response.conversation_context ?? conversationContext);
      if (result?.session_id) {
        setActiveSessionId(result.session_id);
        void refreshChatSessions(accessToken!, authenticatedUserId!);
      }
    },
          ...
          {auth.loading ? (
            ...
            <>
              ...
              <ChatSessionList
                ...
                isSending={isSending}
                onOpen={(sessionId) => void loadStoredSession(sessionId)}
                onDelete={(session) => void deleteStoredSession(session)}
                trashIcon={<Icon name="trash" size={14} />}
              />
              {deleteStatus && (
