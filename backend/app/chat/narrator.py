...
# 인텐트별 상위 모델 라우팅(2026-07-26 실측). 세액 계산은 사용자가 금액을
# 그대로 신뢰해 행동하고, 계좌 비교는 자문 경계를 문장으로 그어야 해서
# 상위 모델의 용어 정의·경계 서술 품질이 값을 한다. 그 밖의 단문 사실
# 전달은 기본 모델이 품질 차이 없이 약 2배 빠르다.
UPGRADED_INTENTS = frozenset(
    {
        ChatIntent.PENSION_TAX,
        ChatIntent.ACCOUNT_RULE,
    }
