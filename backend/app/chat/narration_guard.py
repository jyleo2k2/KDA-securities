...
_UNSAFE_CLAIM_PATTERNS = (
    ...
    (
        ...
        re.compile(
            ...
            # 앞의 원금·수익 주장을 부정한 뒤 "실제로 보장됩니다"처럼
            # 독립된 완료형 보장 주장을 덧붙이는 우회를 별도로 잡는다.
            r"|(?:보장|확정)\s*(?:됩|돼)|확실\s*(?:합|해)"
        ),
    ),
    (
        "recommendation",
...
_NEGATION = re.compile(
    ...
    r"할\s*수\s*없|될\s*수\s*없|드리\s*지\s*(?:않|못)|"
    r"한다\s*고\s*볼\s*수\s*없|(?:해서는|하면|해도)\s*안\s*(?:돼|되)|"
