...
def _knowledge_topic(
    ...
    if re.search(r"담보\s*대출|담보.{0,8}대출", message):
        return (
            "withdrawal_requirements",
            "IRP DC형 담보대출과 중도인출 차이 확인 기준",
            "퇴직연금 수령·중도인출",
            "3. 담보대출과 중도인출은 다르다",
            "계좌 적립금",
        )
    ...
    # 계좌 안에서 무엇을 담고 바꿀 수 있는지. 예금자보호나 DB형 설명이
    # 끼어들지 않도록 투자 가능 자산 매트릭스로 고정한다.
    if re.search(
        r"(?:상품|펀드|ETF).{0,10}(?:바꾸|바꿀|바꿔|교체|변경|갈아타)"
        r"|(?:바꾸|바꿀|바꿔|바꾸면|교체|변경).{0,10}수수료"
        r"|(?:무엇|뭐|어떤).{0,10}(?:담|투자할|살\s*수|매수)"
        r"|투자\s*가능|담을\s*수"
        r"|(?:바로|당장|만들면).{0,10}(?:투자|매수|운용)"
        r"|(?:투자|매수|운용)(?:할|해도)\s*(?:수\s*)?(?:있|돼|되)",
        message,
        flags=re.I,
    ):
        return (
    ...
    # 납입을 쉬는 것은 계좌 해지가 아니며, 계좌별 재개 기준이 답이다.
    if re.search(
        ...
        return (
            "tax_limit",
            "연금저축 납입 중단 재개와 IRP 계좌 유지 기준",
            "연금 기초",
            "납입을 쉬거나 다시 시작할 때",
            None,
        )
    # 몰아서 납입하거나 한도를 넘기는 경우에는 연간 납입·공제 한도가 답이다.
    if re.search(
    ...
    # "돈 필요하면 뺄 수 있어?"는 중도인출 질문이다. 법정 사유가 답이다.
    if re.search(
        r"(?:돈|자금|목돈).{0,14}(?:빼|뺄|찾|인출|쓸|써야|필요)"
    ...
    # 일시금으로 다 받아도 되는지. 수령 방식에 따른 과세가 답이다.
    if re.search(
    ...
    # 이직·퇴직으로 회사를 떠날 때의 퇴직급여 이전. 금융회사를 바꾸는
    # 실물이전과 절차가 다르므로 갈라낸다. 주제를 못 박지 않으면
    # "회사 옮기면 퇴직연금 어떻게 돼?"에 가입자 교육 문서가 나온다.
    if re.search(
        r"(?:회사|직장).{0,6}(?:옮|이직|바꾸|그만|퇴사)"
        r"|이직|퇴사|퇴직할\s*때"
        r"|퇴직급여.{0,8}(?:이전|옮)",
        message,
    ):
        return (
            ...
            "퇴직급여 IRP 이전 원칙과 일시금 예외 확인",
            "퇴직급여의 IRP 이전",
            "2. IRP 이전이 원칙인 상황과 예외",
            None,
        )
    # 금융회사를 바꾸는 계좌 이전. 상품을 팔아야 하는지가 핵심 궁금증이다.
    if re.search(
        r"실물\s*이전"
        r"|(?:증권사|은행|금융\s*회사).{0,10}(?:옮|이전|바꾸|갈아타)"
        r"|계좌.{0,8}(?:옮|이전|이체|갈아타)"
        r"|(?:DC|IRP|연금저축)[^?]{0,10}으?로\s*(?:옮|이전|갈아타)"
        r"|(?:옮|이전).{0,10}상품.{0,10}(?:팔|현금화|해지)",
        message,
        flags=re.I,
    ):
        return (
            "in_kind_transfer",
            "퇴직연금 실물이전 절차와 이전 가능 상품 사전조회",
            "퇴직연금 실물이전",
            "모든 상품이 그대로 옮겨지는 것은 아니다",
            None,
        )
    # 세액공제라는 말 없이 한도만 묻는 경우. "합쳐서 얼마야?"가 대표적이다.
    if re.search(
        r"(?:최대|한도|얼마|합쳐|합산).{0,14}(?:넣|납입|저축|입금)"
        r"|(?:넣|납입|저축|입금).{0,14}(?:최대|한도|얼마)"
        r"|납입\s*한도|연말\s*정산",
        message,
    ):
        return (
...
def _rebalancing_items(evaluation: EducationalPortfolioEvaluation) -> list[str]:
    ...
    parts.append(
        "정해 둔 비율에서 너무 멀어진 자산은 비율 맞추기(리밸런싱)를 "
        "살펴봐요."
    )
    if rebalancing.contribution_first:
        parts.append("팔기보다 새로 넣는 돈을 적은 쪽에 먼저 나눠요.")
