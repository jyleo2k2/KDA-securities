...
def run_live_ingestion(
    *,
    ...
    with httpx.Client(
        ...
        ps_handle = repository.start_run(PS_SOURCE, ps_params) if repository else None
        try:
        ...
        rp_handle = repository.start_run(RP_SOURCE, rp_params) if repository else None
        try:
            ...
            if repository and rp_handle:
                repository.complete_retirement(
                    handle=rp_handle,
