...
async def _fetch_with_retry(
    ...
    *,
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            return await fetch_krx_index_daily_async(
                client,
                api_key=api_key,
                series=series,
                base_date=base_date,
    ...
    if last_error is None:
        raise RuntimeError("KRX index retry loop completed without a result")
    raise last_error
...
def build_benchmark_history(
    *,
    ...
    missing_raw_count = 0
    for base_date in _weekdays(start_date, end_date):
