...
- 파일 수정 전에 `git-session-manager`로 담당자와 `supabase/**`·연결 코드의 예상 수정 경로를 claim한다. 루트 `main` 관제 워크트리에서는 수정하지 않는다.
- 최신 `origin/main`에서 만든 `db/<owner>/<task>` 브랜치·전용 워크트리를 사용하고 첫 커밋 후 Draft PR을 연다. 병합 브랜치를 후속 migration에 재사용하지 않는다.
