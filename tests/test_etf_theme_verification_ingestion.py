...
def test_theme_evidence_covers_all_115_approved_question_types() -> None:
    ...
    assert len(manifest.themes) == 23
    assert len(manifest.themes) * len(ETF_THEME_CONTENT_TOPICS) == 115
...
def test_theme_evidence_validate_only_cli(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "115개 질문 유형" in capsys.readouterr().out
