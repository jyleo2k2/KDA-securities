...
def test_daily_news_workflow_runs_atomic_market_news_pipeline() -> None:
    ...
    assert "GOOGLE_API_KEY: ${{ secrets.GOOGLE_API_KEY }}" in workflow
    assert "NEWS_SUMMARY_MODEL: gemini-3.6-flash" in workflow
    assert "ANTHROPIC_API_KEY" not in workflow
    assert "uv sync --locked --group embeddings" in workflow
    assert "NEWS_SUMMARY_PROMPT_VERSION: news-summary-v4" in workflow
