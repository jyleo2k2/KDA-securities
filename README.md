...
| 이름 | 담당 |
|---|---|
| **이재용** | 프로젝트 총괄 — 전체 아키텍처, 규칙 엔진·LLM 분리, 챗봇·RAG, 서비스 통합·검증, 머지 승인 |
| 이호연 | 조장·챗봇 백엔드 — 뉴스·ETF 테마·연금계좌 챗봇 도구, 세액공제 계산 연동, 데이터 검증 |
| 김태형 | 연금 엔진·ETF 데이터 — 계좌 규칙, CMA 계획가정, 위험·스트레스, 공식 ETF 데이터 |
| 최호택 | 프론트엔드 — React PWA, 로그인·온보딩, 챗봇·홈 화면 |
| 진재현 | 프론트엔드 — React PWA, 로그인·온보딩, 챗봇·홈 화면 |
| 정인성 | 데이터·서비스 통합 — 고객 데이터, 대표 6인 시나리오, Supabase/Auth, QA |

상세 담당 범위와 작업 경계는 [AI 공통 규칙 §5](./docs/team/_공통_AI규칙.md#5-팀-구성담당-2026-07-15-확정)를 따른다.
