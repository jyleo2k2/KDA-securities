...
루트에서 `dev.bat`을 실행하면(내부적으로 `uv run python scripts/dev.py`) 백엔드(8000)·프론트엔드(5173)를 동시에 띄우고 준비 확인 후 브라우저를 연다. 아래는 개별 실행 방법이다.
...
- 원격 Supabase(KDA-securities) 연동 완료: 데이터 기반·권한·챗봇 idempotency/RLS·BGE-M3 차원 고정 마이그레이션과 seed 적용(한국어 인코딩 손상 복구 포함), FSS 실적재(연금저축 2025Q3 88건·퇴직연금 2026Q1 42개사 126행, 수집 이력 성공 기록), NAVER 뉴스 적재, RAG 전문검색·공시·뉴스·챗봇 REST를 실데이터 E2E로 검증.
...
- NAVER 뉴스 원문 임시 추출·Claude 3줄 요약·재시도 큐·최근 5일 무작위 3건 챗봇 응답은 로컬 구현·계약 검증을 완료했다(2026-07-16). 일일 수집·요약 GitHub Actions cron(매일 03:17 KST)도 준비됐다. 새 마이그레이션 원격 적용과 실제 기사·챗봇 E2E는 승인 후 진행한다.
