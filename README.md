...
- `DATABASE_URL`이 있고 원격 실적재가 끝나면 FSS 회사·사업자 공시와 저장 뉴스 조회가 활성화된다. 증시 뉴스는 최근 5일의 요약 완료 기사 중 선별 점수·발행시각 순으로 최대 3건을 `3줄 요약 → 원문 링크` 순서로 제공하며, 통합 조회는 한국·미국 기사를 우선 한 건씩 포함한다.
...
- NAVER 증시뉴스 원문 임시 추출·Claude 3줄 요약·규칙 선별·결정론적 최대 3건 챗봇 응답은 로컬 구현·계약 검증을 완료했다(2026-07-17). 새 마이그레이션 원격 적용과 실제 기사·챗봇 E2E는 승인 후 진행한다.
