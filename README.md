...
- 로컬 검증: `uv run pytest` 180건(시뮬레이션 골든표 28케이스 포함), Ruff, 마이그레이션·seed·내장 SQL 구문 검사 통과.
...
- 엔진 잠정 파라미터(팀 승인 대기): 성향 설문 배점, 진단 임계값, 물가 가정.
- 다음 작업:
  1. 홈(통합 원그래프·진단)·벤치마크 탭 데이터 연결
  2. 시나리오 엔진·공시 조회 중복 구현 통합
  3. 엔진 잠정 파라미터 승인, 한국어 검색 품질 개선
  4. 실제 Auth·audited 경로 종단간 검증, `.claude/settings.json` 자동 검증 훅
